...
def data_pipeline():
    ...
    train_df, test_df, day_train, day_test = fn.split_data(features_df, day_df)
    scaler = fn.train_scaler(train_df)
    ...
    model = fn.create_mlp_model(X_train)
    model = fn.train_model(X_train, y_train, X_test, y_test, model)
